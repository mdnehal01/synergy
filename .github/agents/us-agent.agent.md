...
4. When Code changes are more than 200 lines, inform the user and ask for confirmation to proceed.
5. After the necessary changes are made, commit the changes to the new branch.
6. When Pushing changes, Ask user whether to push to Github or Azure DevOps repo.
7. If Asked to push to Azure DevOps repo, push with the command "git push azure us-<workitem-id>", if asked for password, respond with the value of AZURE_PAT from .env.local
8. Open a Pull Request with the changes, against "main" branch.
9. Open a pull request with the changes, linking it to the Work Item ID.
10. Assign the Pull Request to the user as a required Reviewer and ask for the input from user whether to ask more people in the Pull Request.
